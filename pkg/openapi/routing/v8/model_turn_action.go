...
package routingv8
// TurnAction An action in which a direction change is made.  * `ramp`: ramp maneuver, such as \"Take the left ramp onto\" * `roundaboutPass`: Going straight on a roundabout maneuver * `roundaboutEnter`: Arriving at a destination roundabout maneuver * `uTurn`: u-turn maneuver, such as \"Make a right U-turn at\" * `turn`: turn maneuver, such as \"Make a sharp right turn at\" * `keep`: a maneuver following a fork, such as \"Take the middle fork onto\" * `enterHighway`: a maneuver to enter a highway, used for turn-by-turn guidance only 
